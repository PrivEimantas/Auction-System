...
public class Replica implements ReplicaInterface{
   ...
   boolean isPrimaryReplica;
      ...
      public Replica() {
         ...
         replicaAge=0; //initially is zero.. age depends on when it was booted up, gets set equal to oldest replica acting
     ...
     public Integer newAuction(int userID, AuctionSaleItem item,String token) throws RemoteException{ //returns the auctionID
         ...
         if(AuctionedItems[AuctionedItemsPosition]==null){ //all auctioned items initially null
         ...
         //   System.out.println("newauction 3");
            newAuctionItem.name = UserEmails.get(userID); //Name of highest bidder at that time, initially the person who created
            newAuctionItem.highestBid = item.reservePrice;
            System.out.println("User creating the auction:"+ UserEmails.get(userID));
            newAuctionItem.description = UserEmails.get(userID);// Bid Creator
            ...
            AuctionedItemsPosition++;
            System.out.println("cant place item because an item is already there");
   ...
   public AuctionResult closeAuction(int userID, int itemID,String token) throws RemoteException{
        ...
        // one who created it..
         ...
         //Onur's code might return null because my client is built to have only one email attatched at a time and cant be changed
         //If method checks if item trying to retrieving isnt null and the user trying to close is the same user who made it
         // An item's description is the name of the user who created the auction
         // So the userId extracts from list of emails which is just a string and should be the same as .description
         System.out.println("Close auction printing userID: "+ UserEmails.get(userID));
         System.out.println("ID who created the auction item: "+AuctionedItems[itemID].description);
         ...
         if( AuctionedItems[itemID]!=null && AuctionedItems[itemID].description.equalsIgnoreCase(UserEmails.get(userID))  ){
            ...
            AuctionedItems[itemID] = null; //sets to null because that bid item is not being used anymore
            //On return, print out values which tell who won
            
            return result;
         }
         else{
            System.out.println("Auctioned tried to be closed by someone who did not create auction");
            return null;
         }
